...
arma::rowvec rmvnorm(arma::rowvec mu, arma::mat sigma) {
  ...
  // Correct the covariance matrix: make it positive definite
  arma::vec eigenval;
  arma::mat eigenvec;
  arma::eig_sym(eigenval, eigenvec, sigma);
  
  // I have seen that the arma::chol function fails even with eigenvalues that are positive but very close to zero (usually with an order of 10^-6).
  // Hence I check the condition "< 10^-6" instead of "< 0" 
  if (arma::any(eigenval <= 1e-6)){
    
    arma::uvec indices = arma::find(eigenval <= 1e-6); // the arma::find function returns an arma::uvec object
    int s = indices.size();
    for (int i = 0; i < s; ++i) {
...
List gauss_path_1(int N, arma::vec grid){
  ...
  double s = sqrt((pow(mu,2)+pow(x,2))/2 - pow((mu+x)/2,2)); // define and compute the sd outside the loop because I need it inside of it (at this stage
                                                             // mu is just the first observation and x is the second)
  ...
  for (int i = 2; i < N; ++i){
    ...
    // compute in this iteration, although used in the next one, because I need the "old" mu and not the updated version
...
// The following function produces a path for the copula-based predictive pdf/cdf for a sequence of weights different from the
// one used by Walker. Instead of using alpha_n = (2 - 1/n)/(n+1), we use alpha_n = (2 - 1/n)/((n+1) * (log(n+1))^2).
// We do it only for d = 1.
