...
// Compute the Cholesky decomposition of 1x1 and 2x2 matrices. In particular, it return an upper-triangular matrix that, if multiplied by its transpose, yields the inputed matrix.
// [[Rcpp::export]]
...
// Compute the predictive density and cdf for the Gaussian predictive on a grid of points at each iteration of the predictive resampling algorithm for a given N and B = 1. The
...
// Same as the above function but with the copula-based predictive distribution. Weights of this predictive distribution are taken as in "Martingale posterior distributions" (Fong,
// Holmes and Walker), i.e. alpha_n = (2 - 1/n)/(n+1) for n = 1,2,... . Please note that here weights are called alpha_n while in our paper we name them r_n.
// [[Rcpp::export]]
...
// The following function performs the same task as the above function but different weights, i.e. we use alpha_n = (2 - 1/n)/((n+1) * (log(n+1))^2) for n = 1,2,... .
// [[Rcpp::export]]
